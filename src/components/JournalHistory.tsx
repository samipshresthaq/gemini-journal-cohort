...
export const JournalHistory: React.FC<JournalHistoryProps> = ({
  ...
  // Pagination state for infinite scrolling (10 entries per page)
  ...
  // Reset pagination whenever search query, filters, or entries dataset changes
  useEffect(() => {
    setVisibleCount(PAGE_SIZE);
  }, [
    searchQuery,
  ...
  // Sliced subset of entries for infinite scroll pagination
  ...
  const loadMore = useCallback(() => {
    if (isLoadingMore || visibleCount >= filteredEntries.length) return;
    setIsLoadingMore(true);
    setTimeout(() => {
      setVisibleCount((prev) => Math.min(prev + PAGE_SIZE, filteredEntries.length));
      setIsLoadingMore(false);
    }, 200);
  }, [isLoadingMore, visibleCount, filteredEntries.length]);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      },
      {
        ...
        rootMargin: "180px", // Trigger slightly before reaching the absolute end for smooth reading
        threshold: 0.1,
      }
    );
          ...
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2.5 py-0.5 rounded-full font-semibold border border-slate-200 dark:border-slate-700">
              {filteredEntries.length === 0
                ...
                ? `${filteredEntries.length}${isGuest ? `/${maxGuestEntries}` : ""} ${filteredEntries.length === 1 ? "entry" : "entries"}`
                : `Showing ${visibleEntries.length} of ${filteredEntries.length}${isGuest ? `/${maxGuestEntries}` : ""} entries`
              }
            </span>
          </div>
          <button
      ...
      >
        {filteredEntries.length === 0 ? (
          ...
          <>
            ...
            {hasMore && (
              <div
                id="history-infinite-scroll-sentinel"
                ref={sentinelRef}
                className="py-3 flex flex-col items-center justify-center gap-2 text-center"
              >
                {isLoadingMore ? (
                  <div className="flex items-center gap-2 text-xs font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200/80 dark:border-indigo-800/60 px-3.5 py-2 rounded-xl animate-pulse">
                    <Loader2 className="w-4 h-4 animate-spin text-indigo-600 dark:text-indigo-400" />
                    <span>Loading next 10 reflections...</span>
                  </div>
                ) : (
                  <button
                    ...
                    type="button"
                    onClick={loadMore}
                    className="w-full py-2.5 px-4 rounded-xl bg-slate-50 hover:bg-slate-100 dark:bg-slate-800/80 dark:hover:bg-slate-800 border border-slate-200/80 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                  >
                    <ChevronDown className="w-3.5 h-3.5 text-indigo-500" />
                    <span>
                      Load more reflections ({Math.min(PAGE_SIZE, filteredEntries.length - visibleCount)} remaining)
                    </span>
                  </button>
                )}
              </div>
            ...
            {/* All items loaded indicator for lists exceeding 1 page */}
            {!hasMore && filteredEntries.length > PAGE_SIZE && (
              <div
                id="history-pagination-complete"
                className="py-3 text-center flex items-center justify-center gap-1.5 text-xs text-slate-400 dark:text-slate-500 font-medium"
              >
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 dark:text-emerald-400" />
                <span>Showing all {filteredEntries.length} reflections (10 per page)</span>
              </div>
            )}
          </>
        )}
