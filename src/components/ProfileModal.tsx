      ...
      >
        ...
        <div className="sticky top-0 bg-white/95 backdrop-blur-md px-6 py-4 border-b border-slate-100 flex items-center justify-between z-10">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-900 leading-none">User Profile</h2>
              <p className="text-xs text-slate-500 mt-1">Account credentials, identity, & journaling statistics</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 relative z-10">
              {user.photoURL ? (
                <img
                  ...
                  className="w-16 h-16 rounded-full border-2 border-indigo-400/40 object-cover shadow-sm ring-4 ring-slate-800"
                  referrerPolicy="no-referrer"
                />
              ) : (
                <div className="w-16 h-16 rounded-full bg-indigo-600 text-white flex items-center justify-center text-xl font-bold border-2 border-indigo-400/40 shadow-sm ring-4 ring-slate-800">
                  ...
                  {isEditingName ? (
                    <form onSubmit={handleSaveDisplayName} className="flex items-center gap-2 w-full mt-1">
                      <input
                        ...
                        className="px-3 py-1 text-sm bg-slate-800 border border-indigo-400/50 rounded-lg text-white placeholder-slate-400 focus:outline-hidden focus:ring-2 focus:ring-indigo-400 flex-1"
                        autoFocus
                      />
                      <button
                        type="submit"
          ...
          <div className="space-y-3">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3 text-xs">
              <div className="flex items-center justify-between py-1 border-b border-slate-200/60">
                ...
                <div className="flex items-center gap-2">
                  <code className="text-slate-800 bg-white px-2 py-0.5 rounded-md border border-slate-200 font-mono text-[11px]">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="bg-white border border-slate-200/80 p-3.5 rounded-xl text-center space-y-1 shadow-2xs">
                <div className="w-7 h-7 mx-auto rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center">
                  <BookOpen className="w-3.5 h-3.5" />
                </div>
                <div className="text-xl font-extrabold text-slate-900">{totalEntries}</div>
                <div className="text-[11px] text-slate-500 font-medium">Total Entries</div>
              </div>

              <div className="bg-white border border-slate-200/80 p-3.5 rounded-xl text-center space-y-1 shadow-2xs">
                <div className="w-7 h-7 mx-auto rounded-lg bg-sky-50 text-sky-600 flex items-center justify-center">
                  <MessageSquare className="w-3.5 h-3.5" />
                </div>
                <div className="text-xl font-extrabold text-slate-900">{totalMessages}</div>
                <div className="text-[11px] text-slate-500 font-medium">AI Exchanges</div>
              </div>

              <div className="bg-white border border-slate-200/80 p-3.5 rounded-xl text-center space-y-1 shadow-2xs">
                <div className="w-7 h-7 mx-auto rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center">
                  <Compass className="w-3.5 h-3.5" />
                </div>
                <div className="text-xl font-extrabold text-slate-900">{totalSummaries}</div>
                <div className="text-[11px] text-slate-500 font-medium">Summaries</div>
              </div>
              ...
              <div className="bg-white border border-slate-200/80 p-3.5 rounded-xl text-center space-y-1 shadow-2xs">
                ...
                <div className="text-xl font-extrabold text-slate-900">{favoriteEntries}</div>
                <div className="text-[11px] text-slate-500 font-medium">Starred</div>
              </div>
            </div>
            ...
            {topMoods.length > 0 && (
              <div className="bg-slate-50 border border-slate-200/80 p-3 rounded-xl flex items-center justify-between text-xs">
                <span className="text-slate-500 font-medium">Frequent Moods:</span>
                <div className="flex items-center gap-1.5 flex-wrap">
                  {topMoods.map(([mood, count]) => (
                    <span
                      key={mood}
                      className="px-2.5 py-0.5 rounded-full bg-white border border-slate-200 text-slate-700 font-medium shadow-2xs"
                    >
                      {mood} <span className="text-slate-400 text-[10px]">({count})</span>
                    </span>
                  ))}
                </div>
              </div>
        ...
        {/* Footer Actions */}
        <div className="sticky bottom-0 bg-slate-50 px-6 py-4 border-t border-slate-200 flex items-center justify-between gap-3">
          <button
            ...
            className="px-4 py-2 rounded-xl text-rose-600 hover:bg-rose-50 border border-rose-200 text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
          >
